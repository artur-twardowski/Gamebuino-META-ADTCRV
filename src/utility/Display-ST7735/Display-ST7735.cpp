	...
	if ((img.colorMode == ColorMode::index) && (w == _width) && (h == _height)) {
		...
		for (uint16_t j = 0; j < h; j++) { //vertical coordinate in source image, start from the second line
            ...
            if (img.colorMode == ColorMode::index) {
                Color* palette;
                if (colorCells.enabled) {
                    palette = colorCells.palettes[colorCells.paletteToLine[j]];
                } else {
                    palette = colorIndex;
                }
                for (uint16_t i = 0; i < length / 4; i++) {
                    ...
                    dest[destIdx++] = swap_endians_16((uint16_t)palette[index1]);
                    dest[destIdx++] = swap_endians_16((uint16_t)palette[index2]);
                    dest[destIdx++] = swap_endians_16((uint16_t)palette[index3]);
                    dest[destIdx++] = swap_endians_16((uint16_t)palette[index4]);
                }
            }
